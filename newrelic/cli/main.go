...
func isBoolKey(key string) bool {
	...
	case "NEW_RELIC_ENABLE_BROWSER",
		"ENABLE_NRDOT", "NRDOT",
		"ENABLE_NRI_BUNDLE", "NRI_BUNDLE",
		"ENABLE_DEMO_OTEL_COLLECTOR",
		"ENABLE_PCG", "PCG",
		"ENABLE_APM_TEST_APPS",
		"ENABLE_APM_NATIVE_APPS",
		"ENABLE_APM_HYBRID_APPS":
		return true
	}
	return false
}

func isBoolVal(s string) bool {
	lower := strings.ToLower(s)
	return lower == "true" || lower == "false" || lower == "y" || lower == "n" || lower == "1" || lower == "0"
}
	...
	for i := 0; i < len(args); i++ {
		...
		if strings.HasPrefix(arg, "-") {
			...
			if len(parts) == 2 {
				...
				if i+1 < len(args) && isBoolVal(args[i+1]) {
					val = args[i+1]
					i++
				} else {
					val = "true"
